...
LiFiNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  //std::cout<<"bool LiFiTxNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber) receiverModel "<<" protocolNumber "<<protocolNumber<<std::endl;
...
{
  //std::cout<<"LiFiNetDevice::SendFrom (Ptr<Packet> packet, const Address& source, const Address& dest, uint16_t protocolNumber)"<<std::endl;
  NS_LOG_FUNCTION_NOARGS ();
  NS_ASSERT (Mac48Address::IsMatchingType (dest));
