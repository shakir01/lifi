...
void CentralizedController::TransferApplicationLiFiToWiFi(Ptr<UserDeviceDataModel> userDeviceDataModel)
{
   ...
   if (isRouteRemoved)
   {
        ...
        if (ind_pair.first != -1)
        {
            ...
            /// lifistaticRouting->AddNetworkRouteTo (ipv4,Ipv4Mask("/24"),wifitolifinetinterfaceContainer.GetAddress(1,0),interfaceIndex,1);

            Simulator::Schedule (Seconds (0.9), &CentralizedController::PrintDroppedPackets,this,userDeviceDataModel);

            /*Ptr<Ipv4> ipv4A2                        = udnode->GetObject<Ipv4> ();//node->GetDevice(2)

            int32_t interfaceIndex2                = ipv4A2->GetInterfaceForDevice (lifitxNetDev);
            Ptr<Ipv4L3Protocol> nodeIpv4L3Protocol1 = apNode->GetObject<Ipv4L3Protocol> ();
            Ptr<Ipv4Interface> ipv4Interface       = nodeIpv4L3Protocol1->GetInterface (interfaceIndex2);
            Ptr<ArpCache> lifiApArpCache           = ipv4Interface->GetArpCache ();
            lifiApArpCache->Flush();
            */
            //std::cout<<"cc ipv4Interface 0 "<<ipv4Interface->GetAddress(0).GetLocal()<<"cc ipv4Interface 1 "<<ipv4Interface->GetAddress(1).GetLocal()<<std::endl;

            /*Ptr<WifiApNetDevice> wifinetDev = ns3::DynamicCast <WifiApNetDevice> (udnode->GetDevice(1));
            ipv4Interface->SetDevice(wifinetDev);


            Ipv4Address ipv4InterfaceAddress = ipv4Interface->GetAddress(0).GetLocal();
            std::cout<<"ipv4InterfaceAddress ipv4InterfaceAddress ipv4InterfaceAddress "<<ipv4InterfaceAddress<<" interfaceIndex1 "<<interfaceIndex1<<std::endl;
            staticRouting->AddHostRouteTo(ipv4InterfaceAddress,ind_pair.second,1);

            Ptr < Ipv4StaticRouting > lifistaticRouting   = GetStaticRouting(ipv4A1);
            for (uint32_t i = 0; i < lifistaticRouting->GetNRoutes(); i++)
            {
                Ipv4RoutingTableEntry routeEntry = lifistaticRouting->GetRoute(i);

                std::cout<<"Destination Network: "<<routeEntry.GetDest()<<" userDeviceDataModel->GetDestinationAddress() "<<userDeviceDataModel->GetDestinationAddress()<<std::endl;
                lifistaticRouting->RemoveRoute(i);
            }*/
             /// lifistaticRouting->AddNetworkRouteTo (ipv4,Ipv4Mask("/24"),wifitolifinetinterfaceContainer.GetAddress(1,0),interfaceIndex,1);
  ...
  Ptr<UserDeviceDataModel> userDeviceDataModel1  = FindUserWithID(sink_node->GetId());
...
//void CentralizedController::TransferDataModelFromLiFiToWiFi (Ptr<UserDeviceDataModel> userDeviceDataModel)
//{
...
/*void CentralizedController::CompleteHandover(Ptr<LiFiApMac> controller, std::vector<Ptr<Node> > devicesVector)
{
