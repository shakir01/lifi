...
double LiFiPropagationLossModel::DoCalcRxPower(double txPowerDbm, ns3::Ptr<ns3::MobilityModel> aTX, ns3::Ptr<ns3::MobilityModel> bRX) const
{
	...
	double txGain = CalculateTxGain(aTX,bRX);
		///std::cout<<"Simulation time:"<<Simulator::Now().GetSeconds()<<" this->m_ConcentratorGain "<<this->m_ConcentratorGain<<" this->m_FilterGain "<<this->m_FilterGain<<" this->m_Area "<<this->m_Area<<std::endl;
    ...
    if (lifiConstantVelModel != 0)
    {
		...
		//std::cout<<"double LiFiPropagationLossModel::DoCalcRxPower rxGain "<<rxGain<<" txGain "<<txGain<<" pRX "<<pRX<<" this->m_Area "<<this->m_Area<<" this->m_ConcentratorGain "<<this->m_ConcentratorGain<<std::endl;
	...
	else if(lifiRandomWaypointModel != 0)
	{
		...
		//std::cout<<"ORWP tx "<<tx<<" rx "<<rx<<std::endl;
		rxGain = CalculateRxGain(aTX,bRX);
		pRX = 11.0* rxGain * txGain* this->m_FilterGain * this->m_ConcentratorGain * this->m_Area;
		//std::cout<<"double LiFiPropagationLossModel::DoCalcRxPower rxGain "<<rxGain<<" txGain "<<txGain<<" pRX "<<pRX<<" this->m_Area "<<this->m_Area<<std::endl;
	}
	else 
	{
	//	Vector tx = aTX->GetPosition();
	...
	double distance = GetDistance(aTX, bRX);
		//std::cout<<"Power pRX "<<pRX<<" distance "<<distance<<std::endl;
...
{ 	
	...
	Vector tx = aTX->GetPosition();
	Vector rx = bRX->GetPosition();
	//std::cout<<"double LiFiPropagationLossModel::GetDistance "<<tx<<" rx "<<rx<<std::endl;
