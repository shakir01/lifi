...
void UserDeviceDataModel::DoDispose (void)
{
    m_sinkApplication = 0;
    m_OnOffApplication = 0;
    m_CentralizedController = 0;
    m_lifiphy = 0;
    DoDispose();
}
...
double UserDeviceDataModel::CalculateInstantThroughput (double time)
{
    ...
    double cur = ((currentRxBytes- lastRx) * (double) 8.0) / (1e3);  
...
void CentralizedController::DoDispose (void)
{
    m_userDataModelMaps.clear();
    m_controller_node = 0;
    DoDispose();
}
...
bool CentralizedController::LiFiAPMeasurementReports(Ptr<NetDevice> netDevice,Ptr<const Packet> packet,uint16_t protocolId, const Address & address)
{
    ...
    //userDeviceDataModel->SetSchedularCheckTime(300);
...
void CentralizedController::KPI_Monitoring()
{
    ...
    for(std::unordered_map <uint32_t, Ptr<UserDeviceDataModel> >::iterator it = m_userDataModelMaps.begin(); it != m_userDataModelMaps.end(); ++it)
    {
    ...
    Simulator::Schedule (MilliSeconds (500), &CentralizedController::KPI_Monitoring,this); 
    }
}
